...
public class ListenerUtils {
    ...
    public void sendAIMessage(Message message) throws Exception {
        ...
        // 3. 濡傛灉涓嶆槸瀹堕暱锛屽苟涓斾笉鏄疉I鎵樼锛岀洿鎺ヨ繑鍥?
        if (!message.getSenderIdentity().equals(MessageConstant.PARENT_IDENTITY)) {
            return;
        }
        ...
        // 濡傛灉鑾峰彇鍒扮殑鍒嗘暟杈惧埌涓寸晫鍊硷紝闇€瑕佷慨鏀圭姸鎬?
        if (aiResponse.getScore() < MessageConstant.NEED_EXPERT_SCORE) {
        ...
        // 5. 鎺ㄩ€佹秷鎭?
        webSocketUtils.sendMessageDTO(aiMessageDTO);
        // 5.1 涓哄綋鍓嶅璇濊缃璇濊疆鏁?
        Integer rounds = chat.getRounds() + 1;
        chatService.update().eq("id", chat.getId()).setSql("rounds=rounds+1").update();
        if (!(rounds % 5 == 0 && rounds > 0)) {
            return;
        }
        // 6. 鍐嶈幏鍙栧埌瀹堕暱鐨勫缓妯?
        GetModelRequest modelRequest = new GetModelRequest();
        ...
        if (model != null) {
            // 7. 鎺ㄩ€佹秷鎭?
            webSocketUtils.sendParentModel(chat.getId(), model);
            // 瀛樺叆鏁版嵁搴?
            LocalDateTime latestMessageTime = getLastMessageTime(messages);
    ...
    public void sendDecisionTrees() {
        ...
        // 2. 鍙戦€佹秷鎭?
        webSocketUtils.sendDecisionTrees(decisionTrees);
    }
    ...
    public void sendInstruction(ExpertsAIMessage message) {
        ...
        // 鍙戦€佽姹?
        httpClientUtils.sendInstruction(request);
    }
